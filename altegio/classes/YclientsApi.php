...
class YclientsApi 
{
     ...
     * @access public
     */
    public function __construct($tokenPartner = null,$file_Log)
    {
        $this->setTokenPartner($tokenPartner);
        $this->file_Log = $file_Log; // Путь к лог файлу
        
       // $this->$dtz = new DateTimeZone('Asia/Almaty');
    }
...
//    }
    ...
    public function getAuth($login, $password)
    {
        $temp = $this->request('auth', [
            'login' => $login,
            'password' => $password,
        ], self::METHOD_POST);
        return $temp['data']['user_token'];
    }
     ...
     * 
     * https://developers.yclients.com/ru/#tag/Onlajn-zapis/operation/Получить%20список%20ближайших%20доступных%20сеансов
     * @param integer $companyId
     * @param integer $staffId - ID сотрудника. Фильтр по идентификатору сотрудника
     * @param \DateTime $datetime - дата (в формате iso8601). Фильтр по дате
     *                              бронирования услуги (например '2005-09-09T18:30')
     ...
     * @throws YclientsException
     */
    public function getBookStaffSeances( 
        $companyId,
        $staffId = null,
        $datetime = null,
        // array $serviceIds = null
...
//        }
        ...
        if ($datetime !== null) { 
           //  $parameters['datetime'] = '2022-07-08';
            // $parameters['datetime'] = $this->toISO($datetime); 
            $parameters['datetime'] = '2022-07-16'; 
        ...
        $this->toLog('Получить список ближайших доступных сеансов. Параметры', $parameters); 
        
        return $this->request("book_staff_seances/$companyId/$staffId"  , $parameters);
    }
    ...
    public function getBookStaff(
        ...
        $this->toLog('Параметры для запроса свободных окон', $parameters);
    ...
    public function postBookCheck($companyId, array $appointments)
    {
        ...
        foreach ($appointments as $appointment) {
            if (!isset($appointment['id'], $appointment['staff_id'], $appointment['datetime'])) {
                $this->toLog('Запись должна содержать все обязательные поля: id, staff_id, datetime.');
            }
        }
    ...
    public function postBookRecord( 
        ...
        if (!isset($person['phone'], $person['fullname'], $person['email'])) {
            $this->toLog('Клиент должен содержать все обязательные поля: phone, fullname, email.');
        }

        $parameters = array_merge($parameters, $person);

        if (!count($appointments)) {
            $this->toLog('Должна быть хотя бы одна запись.');
        }
    ...
    public function postCompany(array $fields, $userToken)
    {
        if (!isset($fields['title'])) {
            $this->toLog('Для создании компании обязательно название компании.');
        }
     ...
     * @throws YclientsException
     * https://api.yclients.com/api/v1/company/{company_id}/staff/{staff_id}
     */
    ...
    public function postRecords( 
        ...
        if ($datetime !== null) {
            $parameters['datetime'] = $this->toISO($datetime);
           //   $parameters['datetime'] = $datetime->format("Y-m-d\TH:i:s"); 
            $this->toLog("Данные на создание заявки datetime", $parameters['datetime']); 
        }
        ...
        $this->toLog("Данные на создание заявки custom_fields",  $custom_fields);
        ...
        if ($custom_color !== null) {
            $parameters['custom_color'] = $custom_color; 
        }

        $this->toLog("Данные на создание заявки", $parameters);
    ...
    {
        // $this->toLog("Данные на получение заявки", $companyId . $recordId . $userToken);
    ...
    {
        $this->toLog("Данные на обновление заявки ", $fields);
    ...
    public function getTimetableDates($companyId, $date, $staffId, $userToken)
    {
        ...
        return $this->request('timetable/dates/' . $companyId . '/' . $date, $parameters,
    ...
    public function getTimetableSeances($companyId, $date, $staffId, $userToken)
    {
       ...
       // $date->format('Y-m-d');
        $this->toLog("Дата на запрос сеансов", $date); 

      //  $this->toLog("Получить список сеансов для журнала companyId $companyId, date $date, staffId $staffId, userToken $userToken");
    ...
    {
        if (!isset($fields['url'])) {
            $this->toLog('Не передан обязательный параметр url');
        }
        if (!isset($fields['active'])) {
            $this->toLog('Не передан обязательный параметр active');
        }
    ...
    protected function request($url, $parameters = [], $method = 'GET', $auth = true)
    {
        ...
        if ($auth) {
            if (!$this->tokenPartner) {
                $this->toLog('Не указан токен партнёра');
            }
...
//            echo "</pre>";
    ...
    //Время в формат ISO8601 c +3 часа
    public function toISO($datetime) {
        $datetime = new DateTime($datetime); 
